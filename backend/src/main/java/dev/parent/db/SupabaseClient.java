...
public final class SupabaseClient {
    ...
    /** Generic authorized GET against the PostgREST API. */
    public JsonNode get(String pathAndQuery) throws IOException, InterruptedException {
        return send(base(pathAndQuery).GET().build());
    }

    /** Plain row insert (used by the scan_log audit trail). */
    public JsonNode postRow(String table, JsonNode row) throws IOException, InterruptedException {
        HttpRequest req = base("/rest/v1/" + table)
                .POST(HttpRequest.BodyPublishers.ofByteArray(Json.MAPPER.writeValueAsBytes(row)))
                .build();
        return send(req);
    }
