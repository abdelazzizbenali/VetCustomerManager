...
class DatabaseConnector {
    ...
    public static void ensureDatabaseExists(Connection connection) {
        try (Statement stmt = connection.createStatement()) {
            ResultSet rs = stmt.executeQuery("SHOW DATABASES LIKE 'vetmsdb'");
            if (!rs.next()) {
        ...
        try (Statement stmt = getConnection().createStatement()) {
            ...
            if (!rs.next()) {
                stmt.executeUpdate("CREATE TABLE IF NOT EXISTS medicines (" +
                        ...
                        "  m_barcode VARCHAR(255) NOT NULL DEFAULT '[UNKNOWN BARCODE]'," +
                        "  m_name VARCHAR(100) NOT NULL DEFAULT '[UNKNOWN NAME]'," +
                        "  m_type VARCHAR(100) NOT NULL DEFAULT '[UNKNOWN TYPE]'" +
                        ")ENGINE=InnoDB");
                stmt.executeUpdate("INSERT IGNORE INTO medicines (m_id, m_name) VALUES(-1, '[UNKNOWN MEDICINE]')");
                stmt.executeUpdate("CREATE TABLE IF NOT EXISTS medicinesInfo (" +
                        "  m_id INT PRIMARY KEY," +
                        "  m_size DECIMAL(10,2) NOT NULL DEFAULT 0," +
                        "  m_full_size DECIMAL(10,2) NOT NULL DEFAULT 0," +
                        "  m_buyPrice DECIMAL(10,2) NOT NULL DEFAULT 0," +
                        "  m_sellPrice DECIMAL(10,2) NOT NULL DEFAULT 0," +
                        "  m_expiryDate DATE NOT NULL DEFAULT '9999-12-31'," +
                        "  m_amount INT NOT NULL DEFAULT 0," +
                        "  m_seller VARCHAR(100) DEFAULT '[UNKNOWN SELLER]'," +
                        "  m_description VARCHAR(1000) DEFAULT '[NO DESCRIPTION]'," +
                        "  is_deleted TINYINT(1) DEFAULT 0," +
                        "  FOREIGN KEY (m_id) REFERENCES medicines(m_id) ON DELETE CASCADE" +
                        ")ENGINE=InnoDB");
                stmt.executeUpdate("INSERT IGNORE INTO medicinesinfo (m_id) VALUE(-1)");
                stmt.executeUpdate("CREATE TABLE IF NOT EXISTS clients (" +
                        "  client_id INT PRIMARY KEY AUTO_INCREMENT," +
                        "  name VARCHAR(100) NOT NULL," +
                        "  phone VARCHAR(20) DEFAULT '[UNKNOWN PHONE]'," +
                        "  clientDescription VARCHAR(1000) DEFAULT '[NO DESCRIPTION]'" +
                        ")ENGINE=InnoDB");
                ...
                stmt.executeUpdate("CREATE TABLE IF NOT EXISTS transactions (" +
                        ...
                        "  q_sold INT NOT NULL DEFAULT 0," +
                        "  amount DECIMAL(10,2) NOT NULL DEFAULT 0," +
                        "  type VARCHAR(512) NOT NULL DEFAULT '[UNKNOWN TYPE]'," +
                ...
                stmt.executeUpdate("CREATE PROCEDURE IF NOT EXISTS SellMedicinePartial(" +
                        ...
                        "        m_size = IF(current_amount > 0, current_size, 0)" +
                        "    WHERE m_id = medicine_id;" +
                        "    COMMIT;" +
                        "END;");
            }
...
class DailyUsagePanel extends JPanel {
    ...
    private final JSpinner amountSpinner = new JSpinner(new SpinnerNumberModel(0.0, 0, 100000, 100));
    ...
    private void initializeUI() {
        ...
        medicineCombo.setRenderer(new DefaultListCellRenderer() {
            ...
            public Component getListCellRendererComponent(JList<?> list, Object value, int index, boolean isSelected, boolean cellHasFocus) {
                if (value instanceof Medicine) {
                    value = ((Medicine) value).name();
                }
                return super.getListCellRendererComponent(list, value, index, isSelected, cellHasFocus);
            }
        });
        amountLabel.setFont(new Font("DejaVu",Font.BOLD,20));
        medicineCombo.addActionListener(_ -> updateMaxQuantity());
        ...
        quantitySpinner.addChangeListener(_ -> {
            ...
            if (selected.amount() != 0 && ((int) quantitySpinner.getValue()) <= selected.amount()) {
                amountSpinner.setValue(((int) quantitySpinner.getValue()) * selected.sellPrice());
            }
        });
    ...
    private void updateMaxQuantity() {
        Medicine selected = (Medicine) medicineCombo.getSelectedItem();
        if (selected != null) {
            quantitySpinner.setModel(new SpinnerNumberModel(0, 0, selected.amount(), 1));
            amountSpinner.setValue(selected.sellPrice());
            amountLabel.setText("On Stock : " + selected.amount());
        }
    }
    ...
    private void saveTransaction() {
        try {
            TransactionDAO.addTransaction(
                    ((Client) Objects.requireNonNull(clientCombo.getSelectedItem())).id() ,
                    ((Medicine) Objects.requireNonNull(medicineCombo.getSelectedItem())).id(),
                    LocalDateTime.now(),
...
class MedicinePanel extends JPanel {
    ...
    private void updateInfoPanel(int id) {
        ...
        try (ResultSet rs = MedicineDAO.getMedicineInfo(id)) {
            try {
                if (rs.next()) {
            ...
            showError("Error loading medicine: " + e.getMessage());
        }
    }
...
record Medicine(int id, String name, double currentSize, double fullSize, String type, double buyPrice,
                double sellPrice, LocalDate expiryDate, int amount) {
    @Override
...
record Client(int id, String name, String phone, String description) {
    @Override
    public String toString() {
        return name;
    }
...
class MedicineDialog extends JDialog {
    ...
    public MedicineDialog(Frame owner, String title, int editId, String barcode) {
        ...
        if (editId != -1) {
            try {
                ...
                if (medicine != null) {
                    ...
                    sizeSpinner.setValue(medicine.currentSize());
                    typeField.setSelectedItem(medicine.type());
                    buySpinner.setValue(medicine.buyPrice());
                    sellSpinner.setValue(medicine.sellPrice());
                    datePicker.setDate(Date.valueOf(medicine.expiryDate()));
                    amountSpinner.setValue(medicine.amount());
                }
            } catch (SQLException e) {
        ...
        if (editId != -1) {
            try {
                ...
                if (client != null) {
                    ...
                    descriptionField.setText(client.description());
                }
            } catch (SQLException e) {
...
class TransactionDialog extends JDialog {
    ...
    private void initializeUI() {
        ...
        quantitySpinner.addChangeListener(_ -> {
            Medicine selected = (Medicine) medicineCombo.getSelectedItem();
            assert selected != null;
            if (((double) quantitySpinner.getValue() / selected.fullSize()) != 1) {
                amountSpinner.setValue(((double) quantitySpinner.getValue() / selected.fullSize()) * selected.sellPrice());
            } else {
                amountSpinner.setValue(selected.sellPrice());
            }
        });
    ...
    private void saveTransaction() {
        LocalDateTime transactionDate = LocalDateTime.parse(dateField.getText().trim(), DateTimeFormatter.ofPattern("yyyy-MM-dd, HH:mm:ss"));
        ...
        try {
            TransactionDAO.addTransaction(clientId, selectedMedicine.id(), transactionDate, quantity, (Double)amountSpinner.getValue(), Objects.requireNonNull(typeField.getSelectedItem()).toString(), descriptionArea.getText(), payerCheckbox.isSelected());
            dispose();
        } catch (SQLException e) {
    ...
    private void updateMaxQuantity() {
        Medicine selected = (Medicine) medicineCombo.getSelectedItem();
        if (selected != null) {
            quantitySpinner.setValue(selected.fullSize());
            amountSpinner.setValue(selected.sellPrice());
        }
    }
}
class AppointmentDialog extends JDialog {
    ...
    private void initializeUI() {
        ...
        if (!isTaken) {
            ...
            clientCombo.setRenderer(new DefaultListCellRenderer() {
                public Component getListCellRendererComponent(JList<?> list, Object value, int index, boolean isSelected, boolean cellHasFocus) {
                    return super.getListCellRendererComponent(list, (value instanceof Client) ? ((Client) value).name() : value, index, isSelected, cellHasFocus);
                }
            });
            loadClients();
    ...
    private void addAppointment() {
        ...
        dispose();
        try {
            AppointmentDAO.addAppointment(client.id(), date, description);
        } catch (SQLException e) {
...
class BarcodeScanner {
    public BarcodeScanner() {
        new Thread(() -> {
            ...
            while (true) {
                ...
                String code = detectBarcode(image);
                if (code != null) {
                    if (isBarcodeInDatabase(code)) {
                        playSound("res/Found.wav");
                    } else {
                        playSound("res/notFound.wav");
                        showAddItemDialog(code);
                    }
                    try {
                        Thread.sleep(3000);
    ...
    public static String detectBarcode(BufferedImage image) {
    ...
    public static boolean isBarcodeInDatabase(String code) {
        try (Connection conn = DatabaseConnector.getConnection()) {
            PreparedStatement stmt = conn.prepareStatement("SELECT COUNT(*) FROM medicines WHERE m_barcode = ?");
    ...
    public void showAddItemDialog(String barcode) {
        SwingUtilities.invokeLater(() -> {
            new MedicineDialog(null, "dd Medicine", -1, barcode).setVisible(true);
        });
    }
}
